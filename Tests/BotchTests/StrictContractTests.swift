...
@Suite struct BrowserSessionFileTests {
    private func temporaryFile() -> BrowserSessionFile {
        let folder = FileManager.default.temporaryDirectory
            .appendingPathComponent("botch-session-\(UUID().uuidString)", isDirectory: true)
...
@Suite @MainActor struct MockEnvironmentTests {
    @Test func absentVariablesDisableTheMockEnvironment() {
        unsetenv("BOTCH_MOCK_CHROME")
        unsetenv("BOTCH_MOCK_OPEN")
        #expect(MockEnvironment.chromeRoot == nil)
        #expect(MockEnvironment.openSeconds == nil)
        #expect(MockEnvironment.defaults == nil)
        #expect(MockEnvironment.browserStore(defaults: .standard) == nil)
    }

    @Test func variablesAreParsedIntoADirectoryURLAndSeconds() {
        setenv("BOTCH_MOCK_CHROME", "/tmp/mock chrome", 1)
        setenv("BOTCH_MOCK_OPEN", "2.5", 1)
        defer {
            unsetenv("BOTCH_MOCK_CHROME")
            unsetenv("BOTCH_MOCK_OPEN")
        }
        #expect(
            MockEnvironment.chromeRoot
                == URL(fileURLWithPath: "/tmp/mock chrome", isDirectory: true))
        #expect(MockEnvironment.chromeRoot?.hasDirectoryPath == true)
        #expect(MockEnvironment.openSeconds == 2.5)
        #expect(MockEnvironment.defaults != nil)
        let store = MockEnvironment.browserStore(
            defaults: UserDefaults(suiteName: "botch.mock.test")!)
        #expect(store != nil)
    }

    @Test func unparseableOpenSecondsAreIgnored() {
        setenv("BOTCH_MOCK_CHROME", "/tmp/mock", 1)
        setenv("BOTCH_MOCK_OPEN", "soon", 1)
        defer {
            unsetenv("BOTCH_MOCK_CHROME")
            unsetenv("BOTCH_MOCK_OPEN")
        }
