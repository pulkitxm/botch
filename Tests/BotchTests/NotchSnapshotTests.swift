...
private var snapshotOutputDirectory: URL? {
    ProcessInfo.processInfo.environment["BOTCH_SNAPSHOT_DIR"].map {
        URL(fileURLWithPath: $0, isDirectory: true)
    }
}

@Suite(.serialized, .enabled(if: snapshotOutputDirectory != nil))
@MainActor struct NotchSnapshotTests {

    @Test func rendersTheOpenBrowserWithMockData() async throws {
        let output = try #require(snapshotOutputDirectory)
        ...
        let folder = FileManager.default.temporaryDirectory
            .appendingPathComponent("botch-snapshot-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: folder) }
        let defaults = try #require(UserDefaults(suiteName: "test.botch.snapshot.\(UUID())"))
        ...
        let image = try await NotchSnapshot.capture(panel: panel, webView: inbox.webView)
        try FileManager.default.createDirectory(at: output, withIntermediateDirectories: true)
        try image.write(to: output.appendingPathComponent("botch.png"))
...
@MainActor
enum NotchSnapshot {
    static func capture(panel: NSPanel, webView: WKWebView) async throws -> Data {
        let content = try #require(panel.contentView)
        let shape = try #require((content as? NotchCatcherView)?.interactiveShapeSize)
        let container = try #require(find(BrowserWebContainerView.self, in: content))
        var page: NSImage?
        for _ in 0..<10 {
        ...
        let crop = CGRect(
            x: (content.bounds.width - shape.width) / 2 * CGFloat(scale),
            y: 0, width: shape.width * CGFloat(scale),
        ...
        let final = NSBitmapImageRep(cgImage: cropped)
        return try #require(final.representation(using: .png, properties: [:]))
    }
...
enum MockPages {
    ...
    static let inbox = shell(
        ...
        body: """
            ...
            <div class="row unread"><span class="from">Design Review</span><span>Notch browser mockups are ready</span><span class="when">9:12 AM</span></div>
            <div class="row"><span class="from">Build Bot</span><span>CI passed on main (101 tests)</span><span class="when">8:55 AM</span></div>
            ...
            <div class="row"><span class="from">Mock Docs</span><span>Alex shared "Roadmap Q4" with you</span><span class="when">Sep 22</span></div>
            </section>
            """)
    ...
    static let docs = shell(
        ...
        body: """
            ...
            <p>This is a mock document served from a local fixture. It exists only so the
            screenshot shows a page with text, and it contains no real data.</p>
            <p>Items: ship the notch browser, tidy the settings window, write the release notes,
            and measure memory and CPU with the helper processes listed separately.</p>
            </div></section>
            """)
    ...
    static let calendar = shell(
        ...
        body: """
            ...
            <tr><td>Mon 21</td><td>Tue 22</td><td>Wed 23<div class="ev">Design review</div></td><td>Thu 24</td><td>Fri 25<div class="ev">Release v0.1.0</div></td></tr>
            <tr><td>Mon 28<div class="ev">1:1</div></td><td>Tue 29</td><td>Wed 30</td><td>Thu 1</td><td>Fri 2</td></tr>
            </table></div></section>
            """)
}
