...
@Suite struct ChromeProfileParserTests {
    ...
    @Test func userDataPrefersTheNewestCookieDatabase() throws {
        let folder = FileManager.default.temporaryDirectory
            .appendingPathComponent("botch-chrome-\(UUID().uuidString)", isDirectory: true)
...
@Suite struct NotchBrowserGeometryTests {
    ...
    @Test func thePanelWrapsTheBrowserShapeWithPadding() {
        let shape = CGSize(width: 1000, height: 672)
        let panel = NotchGeometry.panelSize(forShape: shape)
        #expect(panel.width == shape.width + NotchGeometry.panelPadding.width)
        #expect(panel.height == shape.height + NotchGeometry.panelPadding.height)
    }
    ...
    @Test func notchShapeGrowsToTheBrowser() {
        let shape = NotchGeometry.expandedShapeSize(
            browserSize: CGSize(width: 1000, height: 640), notchHeight: 32)
...
@Suite struct NotchBrowserHidePolicyTests {
    @Test func browserStaysOpenThroughSmallOvershoots() {
        let browser = NotchHidePolicy.browser
        #expect(browser.keepInset > NotchGeometry.keepInset)
        #expect(browser.closeGrace > 0.5)
        #expect(browser.trackingMargin > browser.keepInset)
    }
    ...
    @Test func pointerJustPastTheBrowserEdgeKeepsItOpen() {
        ...
        #expect(
            NotchGeometry.proximity(
                point: justBelow, collapsedFrame: collapsed, expandedFrame: expanded,
                keepInset: NotchGeometry.keepInset) == .outside)
    }
